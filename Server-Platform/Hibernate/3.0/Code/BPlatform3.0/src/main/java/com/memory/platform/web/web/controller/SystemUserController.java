...
//				s.setPwd(MD5andKL.KL(s.getPwd()));
	...
	@ResponseBody
	public Object loadUserRoleTree(@RequestParam(value="userId",required=true)String userId,@RequestParam(value="id",required=false)String parentRoleId,HttpServletRequest request,Model model) {
		Map requestMap = request.getParameterMap();
		model.addAllAttributes(requestMap);
	...
	@ResponseBody
	public Object loadUserDeptTree(@RequestParam(value="userId",required=true)String userId,@RequestParam(value="id",required=false)String parentDeptId,HttpServletRequest request,Model model) {
		Map requestMap = request.getParameterMap();
		model.addAllAttributes(requestMap);
