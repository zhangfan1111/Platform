var system = system || {};
system.data = system.data || {};// 用于存放临时的数据或者对象
 ...
 * 增加命名空间功能
 * 
 * 使用方法：system.ns('jQuery.bbb.ccc','jQuery.eee.fff');
 */
system.ns = function() {
	var o = {}, d;
 ...
 * 将form表单元素的值序列化成对象
 * 
 * @example system.serializeObject($('#formId'))
 * 
 * @requires jQuery
 * 
 * @returns object
 */
system.serializeObject = function(form) {
	var o = {};
 ...
 * 增加formatString功能
 * 
 * @example system.formatString('字符串{0}字符串{1}字符串','第一个变量','第二个变量');
 * 
 * @returns 格式化后的字符串
 */
system.formatString = function(str) {
 ...
 * @returns 格式化后的字符串
 */
system.getLastIndex = function(str,sub) {
	return str.substring(str.lastIndexOf(sub)+1);
};
/**
 * 去掉后缀名
 * 
 * @returns 格式化后的字符串
 */
system.getFirstIndex = function(str,sub) {
	return str.split('.')[0];
};
/**
 ...
 * @returns list
 */
system.stringToList = function(value) {
 ...
 * @returns
 */
system.jsonToString = function(o) {
	var r = [];
	if (typeof o == "string")
	...
	if (typeof o == "object") {
		if (!o.sort) {
			for ( var i in o)
				r.push(i + ":" + system.jsonToString(o[i]));
			...
			for (var i = 0; i < o.length; i++)
				r.push(system.jsonToString(o[i]));
			r = "[" + r.join() + "]";
		}
		return r;
	}
 ...
 * 
 * @example system.cookie('the_cookie', 'the_value');
 * @desc Set the value of a cookie.
 * @example system.cookie('the_cookie', 'the_value', { expires: 7, path: '/', domain: 'jquery.com', secure: true });
 * @desc Create a cookie with all available options.
 * @example system.cookie('the_cookie', 'the_value');
 * @desc Create a session cookie.
 * @example system.cookie('the_cookie', null);
 ...
 * 
 * @example system.cookie('the_cookie');
 ...
 * @author Klaus Hartl/klaus.hartl@stilbuero.de
 */
system.cookie = function(key, value, options) {
