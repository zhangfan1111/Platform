...
<script type="text/javascript">
	...
	var addFun = function() {
		var dialog = parent.system.modalDialog({
			...
			border: false,
			height:430,
			url : system.contextPath + '/system/systemDeptController/form',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	var showFun = function(id) {
		var dialog = parent.system.modalDialog({
			...
			border: false,
			height:430,
			url : system.contextPath + '/system/systemDeptController/form?id='+id
		});
	};
	...
	var removeFun = function(id) {
		parent.$.messager.confirm('询问', '您确定要删除此记录？', function(r) {
			if (r) {
				$.post(system.contextPath + '/system/systemDeptController/delete/'+id, {
					id : id
				}, function() {
	...
	var grantFun = function(id) {
		var dialog = parent.system.modalDialog({
			...
			border: false,
			height:430,
			url : system.contextPath + '/system/systemDeptController/grant/id=' + id,
			buttons : [ {
				text : '授权',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm(dialog, grid, parent.$);
	...
	$(function() {
		grid = $('#grid').treegrid({
			title : '',
			url : system.contextPath + '/system/systemDeptController/treeGrid',
			idField : 'id',
			treeField : 'name',
