...
public class BaseDaoImpl<T> implements IBaseDao<T> {
	...
	public <T> SearchResult<T> searchByHql(Search<T> search) {
		SearchResult<T> sResult = new SearchResult<T>();
		sResult.setResult((List<T>) find(search.getHql(),search.getPage(),search.getRows()));
		sResult.setTotalCount(count(search.getCountHql()).intValue());
		
		return sResult;
	}
	public <T> SearchResult<T> searchBySql(Search<T> search) {
		SearchResult<T> sResult = new SearchResult<T>();
		/** 2015-3-9 by norain 修改find为findBySql；count改为countBySql **/
		sResult.setResult((List<T>) findBySql(search.getSql(),search.getPage(),search.getRows()));
		sResult.setTotalCount(countBySql(search.getCountSql()).intValue());
...
>>>>>>> b1a35d51e89904bf0fc3d56ec845346d216c5b6d
