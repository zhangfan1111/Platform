...
NSString * const AccessKey = @"LTAIBu1n6STvMIym";
NSString * const SecretKey = @"q5PIq6KivS2HTXnl3TJiOyTdTRQ1mW";
...
- (void)listObjectsInBucket {
    ...
    getBucket.bucketName = @"wehang-qj";
    getBucket.delimiter = @"";
    getBucket.prefix = @"";
...
- (void)downloadObjectAsync {
    ...
    // required
    request.bucketName = @"yangshaoping";
    request.objectKey = @"0bd162d9f2d3572c21a659248d13632763d0c386.jpg";
    //对图片进行处理
//    request.xOssProcess = @"image/resize,m_lfit,w_100,h_100";
    ...
    [getTask continueWithBlock:^id(OSSTask *task) {
        if (!task.error) {
            ...
            NSLog(@"download dota %@", getResult.downloadedData);
        } else {
...
- (void)copyObjectAsync {
    ...
    copy.bucketName = @"wehang-qj"; // 复制到哪个bucket
    copy.objectKey = @"file_copy_to"; // 复制为哪个object
    copy.sourceCopyFrom = [NSString stringWithFormat:@"/%@/%@", @"wehang-qj", @"file1m"]; // 从哪里复制
...
- (void)multipartUpload {
    ...
    NSString * uploadToBucket = @"wehang-qj";
...
- (void)listParts {
    ...
    listParts.bucketName = @"wehang-qj";
